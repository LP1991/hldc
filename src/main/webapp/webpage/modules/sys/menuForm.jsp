	...
	<script type="text/javascript">
		...
		function doSubmit(index,table,method){//回调函数，在编辑和保存动作时，供openDialog调用提交表单。
			if(validateForm.form()){
				  submitData( '${ctx}/sys/menu/saveA',getFormParams('inputForm'),function(data){
						var icon_number;
						if(!data.success){
							icon_number = 8;
						}else{
...
<body class="hideScroll">
	<form:form id="inputForm" modelAttribute="menu" action="${ctx}/sys/menu/save" method="post" class="form-horizontal" style="margin:0;">
		<form:hidden path="id"/>
		<input id="parent_id_form" type="hidden" name="parent.id"/>
			...
			<tbody>
				<tr>
					...
					<%-- <sys:treeselect id="menu" name="parent.id" value="${menu.parent.id}" labelName="parent.name" labelValue="${menu.parent.name}" 
						...
						<input id="parent_name_form" disabled="disabled" class="required form-control "/>
						</td>
				...
				<tr>
					...
					<td><form:input path="sort" htmlEscape="false" maxlength="50" class="required digits form-control " /> 
						<span class="help-inline">排列顺序，升序。</span></td>
				</tr>
				<tr>
					<td class="active"><label class="pull-right">类别:</label></td>
					<td><form:radiobuttons path="isShow" items="${fns:getDictListLike('menu_type')}" itemLabel="label"
						itemValue="value" htmlEscape="false" class="required i-checks " />
