	...
	<script type="text/javascript">
		...
		function doSubmit(index,table){//回调函数，在编辑和保存动作时，供openDialog调用提交表单。
			if(validateForm.form()){
				 setTimeout(function(){
					  submitData( '${ctx}/sys/role/saveA',getFormParams('inputForm'),function(data){
						  var icon_number;
							if(!data.success){
								icon_number = 8;
							}else{
		...
		$(document).ready(function(){
			$("#name").focus();
			$.validator.addMethod("f_English",function(value,element,params){
			...
			validateForm= $("#inputForm").validate({
				rules: {
                   ...
                   required:true,
					...
					enname: {
						remote: "${ctx}/sys/role/checkEnname?oldEnname=" + encodeURIComponent("${role.enname}"),
					required:true,
					f_English:true
				},
				},
...
<body class="hideScroll">
	<form:form id="inputForm" modelAttribute="role" autocomplete="off" action="${ctx}/sys/role/save" style="margin:0;" method="post" class="form-horizontal" >
		<form:hidden path="id"/>
		   ...
		   <tbody>
		      ...
		      <tr>
		         ...
		         <td class="width-35"><form:select path="dataScope" class="form-control ">
					<form:options items="${fns:getDictListLike('sys_data_scope')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
					</form:select>
