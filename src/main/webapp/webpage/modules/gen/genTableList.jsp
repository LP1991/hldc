...
<body class="gray-bg">
	...
	<div class="ibox-title">
		<h5>表单列表  </h5>（欢迎使用代码生成器，最新版请访问<a target="_blank" href="http://www.hlframe.org">http://www.hlframe.org</a>)
>
	...
	<div class="col-sm-12">
		...
		<button class="btn btn-success btn-rect  btn-sm " onclick="search()" ><i class="fa fa-search"></i> 查询</button>
	</form:form>
	<br/>
	</div>
	</div>
	...
	<div class="col-sm-12">
		<div class="pull-left">
				...
				<button class="btn btn-success btn-rect btn-sm" data-toggle="tooltip" data-placement="left" title="导入" onclick="openDialog('导入表单','${ctx}/gen/genTable/importTableFromDB','700px', '300px')"><i class="fa fa-folder-open-o"></i> 数据库导入表单</button>
			    </shiro:hasPermission>
			    ...
			    <table:delRowByAjax url="${ctx}/gen/genTable/deleteAll" label="移除记录" id="contentTable" ></table:delRowByAjax><!-- 移除记录 -->
			    </shiro:hasPermission>
				<shiro:hasPermission name="gen:genTable:genCode">
	       		<button class="btn btn-success btn-rect btn-sm" data-toggle="tooltip" data-placement="left" title="生成代码" onclick="genCode()"><i class="fa fa-folder-open-o"></i> 生成代码</button>
	       		<button class="btn btn-success btn-rect btn-sm" data-toggle="tooltip" data-placement="left" title="创建菜单" onclick="createMenu()"><i class="fa fa-folder-open-o"></i> 创建菜单</button>
	       		</shiro:hasPermission>
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		...
		<tr>
			...
			<th class="table_type">表类型</th>
			<th class="name">表名</th>
			<th>说明</th>
			<th class="class_name">类名</th>
			<th class="parent_table">主表</th>
			<th class="isSync">同步数据库</th>
			<th>操作</th>
		</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="genTable">
			<tr>
				...
				<td>
					<shiro:hasPermission name="gen:genTable:edit">
					<a href="#" onclick="openDialog('修改业务表', '${ctx}/gen/genTable/form?id=${genTable.id}','1000px', '700px')" class="btn btn-success btn-xs"><i class="fa fa-edit"></i></a>
					</shiro:hasPermission>
					<shiro:hasPermission name="gen:genTable:del">
    				<a href="${ctx}/gen/genTable/delete?id=${genTable.id}" onclick="return confirmx('确认要移除该条记录吗？', this.href)" class="btn btn-success btn-xs" ><i class="fa fa-trash"></i> 移除</a>
    				</shiro:hasPermission>
    				<shiro:hasPermission name="gen:genTable:del">
					<a href="${ctx}/gen/genTable/deleteDb?id=${genTable.id}" onclick="return confirmx('确认要删除该条记录并删除对应的数据库表吗？', this.href)" class="btn btn-success btn-xs" ><i class="fa fa-trash"></i> 删除</a>
					</shiro:hasPermission>
					<shiro:hasPermission name="gen:genTable:synchDb">
					<a href="${ctx}/gen/genTable/synchDb?id=${genTable.id}" onclick="return confirmx('确认要强制同步数据库吗？同步数据库将删除所有数据重新建表！', this.href)" class="btn btn-success btn-xs" ><i class="fa fa-database"></i> 同步数据库</a>
					</shiro:hasPermission>
				</td>
			</tr>
		</c:forEach>
