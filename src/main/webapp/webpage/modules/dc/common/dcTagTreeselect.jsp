...
<head>
	...
	<script type="text/javascript">
		...
		var tree, setting = {view:{selectedMulti:false,dblClickExpand:false,addDiyDom: addDiyDom},check:{enable:"${checked}",nocheckInherit:true},
		...
		function addDiyDom(treeId, treeNode) {
			//if (treeNode.parentNode && treeNode.parentNode.id!=2) return;
				if(treeNode.level==0&&${clearItem}){
					var aObj = $("#" + treeNode.tId + "_a");
					var editStr = "<button class='btn btn-primary'  id='cleartree'>清除选择项</button>";
					aObj.after(editStr);
					var btn = $("#cleartree");
					btn.bind("click", function(){
						confirmx("确认清除选择项吗？", function(){
		...
		$(document).ready(function(){
			$.get("${ctx}${url}${fn:indexOf(url,'?')==-1?'?':'&'}&extId=${extId}&isAll=${isAll}&module=${module}&t="
					+ new Date().getTime()+"&otherParam1=${otherParam1}&otherParam2=${otherParam2}&otherParam3=${otherParam3}&otherParam4=${otherParam4}&otherParam5=${otherParam5}", function(zNodes){
				// 初始化树结构
