...
<body class="hideScroll">
	...
	<form:form id="inputForm" modelAttribute="formData" autocomplete="off" action="#" method="post" class="form-horizontal" >
		<form:hidden path="id"/>
		<form:hidden path="jobType" value="1"/>	<!-- 任务类别: 1-restful webservice; 2-soap webservice -->
			...
			<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
			   <tbody>
				  ...
				  <tr>
					 ...
					 <td class="width-35">
						 <form:input path="restUrl" htmlEscape="false" class="form-control required isURL"/>
						 <span class="help-inline">rest webservice 服务端连接地址</span>
					 </td>
				  </tr>
				  <tr>
					 <td class="width-15 active" ><label class="pull-right">接口调用方式：</label></td>
					 <td class="width-35">
						 <form:select path="restType" class="form-control required" onchange="updateFormParam(this.options[this.options.selectedIndex].value)">
							 <form:option value="post">post方式</form:option>
							 <form:option value="get">get方式</form:option>
						 </form:select>
						 <span class="help-inline">rest webservice 接口调用方式</span>
					 </td>
				  </tr>
				  <tr id="__restContentType">
					 <td class="width-15 active" ><label class="pull-right">表单参数方式：</label></td>
					 <td class="width-35">
						 <form:select path="restContentType" class="form-control required" >
							 <form:option value="application/x-www-form-urlencoded">(默认)x-www-form-urlencoded</form:option>
							 <form:option value="multipart/form-data">multipart/form-data</form:option>
							 <form:option value="application/json">application/json</form:option>
						 </form:select>
						 <span class="help-inline">rest webservice 服务端接收的表单参数方式</span>
					 </td>
				  </tr>
				  <tr id="__params">
					 ...
					 <td class="width-35">
						 ...
						 <span class="help-inline">rest服务端接收参数, 多个参数以&进行分割,格式:[参数名1=参数值1&参数名2=参数值2]</span>
					 </td>
				  </tr>
				</tbody>
			</table>
				  ...
				  <tr>
					 ...
					 <td class="width-35">
						<dc:treeselect allowInput="false" clearItem="true"
								id="schema" name="schemaName" value="${formData.schemaName}"
								labelName="数据库Schema" labelValue="${formData.schemaName}"
								notAllowSelectParent="true" title="Schema"
								url="/dc/metadata/db/schemaTreeData"
								cssClass="form-control required"
								validateField="connId" otherParam1="connId" />
	...
	<script>
         ...
         * @param selectVal
         */
		function updateFormParam(selectVal) {
			if("get"==selectVal){
			    $('#__restContentType').hide();
                $('#__params').hide();
			}else{
                $('#__restContentType').show();
                $('#__params').show();
			}
        }
        ...
        function updateFlag(obj) {
            obj = obj || document.getElementById('_createFlag');
            debugger;
            if(obj.checked){
                obj.value='Y';
            }else{
		...
		$(document).ready(function() {
			...
			validateForm = $("#inputForm").validate({
				rules: {
					jobName: {remote: "${ctx}/dc/dataProcess/transIntf/checkJobName?jobType=1&oldJobName=" + encodeURIComponent('${formData.jobName}')}//设置了远程验证，在初始化时必须预先调用一次。
                   /* 目标表暂只能选择, 所以不需要重复验证
                   	tarName: {
					    remote: {
					        url: "${ctx}/dc/dataProcess/transIntf/checkTarName",
							type: "post",               //数据发送方式
...
//                    tarName: {remote: "数据表已存在"}
			...
			//控制post参数表单
            updateFormParam($("#restType").find("option:selected").val());
		});
	</script>
</body>
</html>