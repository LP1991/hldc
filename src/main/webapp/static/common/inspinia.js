...
/*$(function () {
    ...
    $.get(rootpath+"/static/common/pt-change.jsp", function (data) {
        $('body').append(data);
 ...
 * webroot=webroot.substring(webroot.indexOf('//')+2,webroot.length);
 * webroot=webroot.substring(webroot.indexOf('/')+1,webroot.length);
 * webroot=webroot.substring(0,webroot.indexOf('/')); rootpath="/"+webroot;
 * 
 * 
 ...
 * 
 * var collapse = localStorage.getItem("collapse_menu"); var fixednavbar =
 * localStorage.getItem("fixednavbar"); var boxedlayout =
 * localStorage.getItem("boxedlayout");
 * 
 * var body = $('body');
 * 
 * if (collapse == 'on') { if (!body.hasClass('body-small')) {
 * body.addClass('mini-navbar'); } }
 * 
 * if (fixednavbar == 'on') {
