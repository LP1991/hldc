...
function closeTab() {
    ...
    if ($(this).parents('.J_menuTab').hasClass('active')) {
        ...
        if ($(this).parents('.J_menuTab').next('.J_menuTab').size()) {
            ...
            found_left( $(this).parents('.J_menuTab').next('.J_menuTab:eq(0)'));
        ...
        if ($(this).parents('.J_menuTab').prev('.J_menuTab').size()) {
            ...
            found_left($(this).parents('.J_menuTab').prev('.J_menuTab:last'));
            $('.J_mainContent .J_iframe').each(function () {
                if ($(this).data('id') == activeId) {
                    $(this).show().siblings('.J_iframe').hide();
                    return false;
...
//寻找到左侧菜单对应的选项，并将其他的去掉
function found_left(a_tab,rule){
	var a = $('#side-menu').find('.J_menuItem');
	if(rule){
		$('#side-menu').find('ul').removeClass('active in');
		$('#side-menu').find('li').removeClass('active');
		$('#side-menu').find('a').removeClass('click-on');
		return;
	}
	if(a_tab.attr('data-id').toString()=='/hldc/page/home'){
		$('#side-menu').find('ul').removeClass('active in');
		$('#side-menu').find('li').removeClass('active');
		$('#side-menu').find('a').removeClass('click-on');
	}
	for(var i=0;i<a.length;i++){
		...
		var tab_href = a_tab.attr('data-id');
		if(a_href == tab_href && a.eq(i).text().replace(/\s+/g, "") == a_tab.text().replace(/\s+/g, "")){
			//由于有些选项的href指向连接相同，所以还需要判断文本是否相同,有的文本标签会加上空格，这里使用正则去除空格，进行比较
			var str = a.eq(i).parent().parent().attr('class').toString();
			if(str.indexOf('third')!==-1){
				$('#side-menu').find('ul').removeClass('active in');
				$('#side-menu').find('li').removeClass('active');
				$('#side-menu').find('a').removeClass('click-on');
				a.eq(i).parent().parent().parent().parent().parent().addClass('active');
				a.eq(i).parent().parent().parent().parent().addClass('in');
				a.eq(i).parent().parent().parent().addClass('active');
				a.eq(i).parent().parent().addClass('in').height('auto');
				
				a.eq(i).parent().addClass('active');
				a.eq(i).addClass('click-on');
				return;
