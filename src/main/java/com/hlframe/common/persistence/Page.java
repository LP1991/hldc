 ...
 */
package com.hlframe.common.persistence;
...
public class Page<T> {
	...
	public Page(HttpServletRequest request){
		...
		if (StringUtils.isNumeric(length)){
			this.setPageSize(Integer.parseInt(length));
		}
		// 设置页面大小参数（传递repage参数，来记住页码大小）
		...
		this.setPageNo(this.start/getPageSize()+1);
		String search = request.getParameter("search");
		/*if (StringUtils.isNumeric(start)){
			this.setPageSize(Integer.parseInt(start));
		}*/
		// 设置排序参数
