 ...
 */
package com.hlframe.common.servlet;
...
public class ValidateCodeServlet extends HttpServlet {
	...
	//是否使用验证码
	public static String useFlag = Global.getConfig("system.validateCode.useFlag");
	...
	public static boolean validate(HttpServletRequest request, String validateCode){
		//登录时不验证 验证码 peijd
		if("false".equalsIgnoreCase(useFlag)){
			return true;
		}
	...
	private void createImage(HttpServletRequest request,
		...
		/*
		 * 生成字符 不使用验证码, 则生成'----' peijd
		 */
		if("false".equalsIgnoreCase(useFlag)){
			request.getSession().setAttribute(VALIDATE_CODE, "----");
		}else{
	...
	private String createCharacter(Graphics g) {
		...
		for (int i = 0; i < 4; i++) {
			String r = String.valueOf(codeSeq[29]);//random.nextInt(10));
