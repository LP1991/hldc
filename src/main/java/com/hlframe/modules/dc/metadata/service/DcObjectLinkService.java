...
public class DcObjectLinkService extends CrudService<DcObjectLinkDao, DcObjectLink> {
	...
	/**
	 * @方法名称: configObjLinkByProcess 
	 * @实现功能: 根据转换任务过程 设置链路关系
	 * @param processId
	 * @param srcTable
	 * @param tarTable
	 * @create by peijd at 2016年12月10日 上午11:39:43
	 */
	@Transactional(readOnly = false)
	public void configObjLinkByProcess(String processId, String srcTable, String tarTable){
		...
		//解析表关系 添加到对象关系表 dc_obj_link
		for(String inTable: srcTable.split(",")){
			link = new DcObjectLink();
			link.setId(IdGen.uuid());
			//数据来源-转换任务
			link.setDataSource(DcObjectLink.DATA_SOURCE_TRANSJOB);
			link.setProcessId(processId);
			link.setLinkType(DcObjectLink.LINK_TYPE_TABLE);
			link.setSrcObjId(inTable);
			link.setTarObjId(tarTable);
			link.setLinkJson(new StringBuilder(64).append("{from:'").append(inTable).append("', to:'").append(tarTable).append("'}").toString());
			linkList.add(link);
			
//			Neo4j neo4j = new Neo4j();
//			neo4j.setTableName(srcTable);
//			neo4j.setTarName(tarTable);
//			neo4j.setProcessId(processId);
