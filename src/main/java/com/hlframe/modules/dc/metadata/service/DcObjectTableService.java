...
public class DcObjectTableService extends CrudService<DcObjectTableDao, DcObjectTable> {
    ...
    public void saveTable(DcObjectTable obj) throws Exception {
    ...
    /**
     * @方法名称: public2Interface
     * @实现功能: 元数据(数据表对象) 发布接口
     * @params  tableIntf	数据表对象
     * @return  void
     * @create by peijd at 2017/5/4 13:56
     */
    @Transactional(readOnly = false)
    public void public2Interface(DcObjectInterface tableIntf){
        Assert.hasText(tableIntf.getIntfcSrcId());

        DcObjectTableInfo table = dao.buildTableInfo(tableIntf.getIntfcSrcId());
        DcObjectMain main = null;
        if(StringUtils.isBlank(tableIntf.getObjId())){
            //判断该数据表是否发布  一个数据表只能发布一个接口
            DcObjectInterface inftParam = new DcObjectInterface();
            inftParam.setIntfcSrcId(tableIntf.getIntfcSrcId());
            Assert.isTrue(CollectionUtils.isEmpty(intfService.findList(inftParam)), "数据表["+ table.getTableName()+"]已发布接口!");
            main = new DcObjectMain();
            main.setId(IdGen.uuid());
        }else{
            main = mainService.getById(tableIntf.getObjId());
        }

        //构建main对象  main & interface
        main.setObjCode("INTF_"+table.getTableName());
        main.setObjName(tableIntf.getObjName());
        main.setObjType(DcObjectMain.OBJ_TYPE_INTER);	//接口对象
        main.setObjDesc(tableIntf.getObjDesc());
        main.setJobSrcFlag(DcObjectMain.JOB_SRC_FLAG_YES);

        DcObjectInterface intf = new DcObjectInterface();
        try {
            MyBeanUtils.copyBean2Bean(intf, tableIntf);
        } catch (Exception e) {
            throw new ServiceException(e);
        }
        //构建接口对象
        intf.setObjId(main.getId());
        intf.setIntfcType(DcObjectInterface.INTFC_TYPE_RESTFUL);	//rest接口
        intf.setIntfcCalltype("post");
        intf.setIntfcContype(MediaType.APPLICATION_FORM_URLENCODED);	//参数表单: x-www-form-urlencoded
        //保存元数据
        getDcMetadataStroeService().obj2MySQL(main, intf);

        //链路关系 元数据 table → interface
        getObjLinkService().configObjLinkByExtractIntf(intf.getObjId(), table.getObjId(), main.getId());
    }
