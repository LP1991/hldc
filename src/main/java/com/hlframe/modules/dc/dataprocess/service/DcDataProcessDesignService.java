...
//		param.setLinkType(DcObjectLink.LINK_TYPE_FIELD);	//字段
	 ...
	 * @param jobId
	 * @return
	 * @create by peijd at 2017年3月7日 下午7:56:17
	 */
	@Transactional(readOnly = false)
	public String add2Schedule(String jobId) {
		...
		DcDataProcessDesign transJob = dao.get(jobId);
		String taskName = "数据转换_"+transJob.getDesignName();
		//调度任务
		...
		//任务描述
		taskMain.setTaskDesc(transJob.getDesignDesc());
		//任务来源
		taskMain.setTaskPath(DcTaskMain.TASK_FROMTYPE_PROCESS_DESIGN);
		//优先级
		...
		//更新记录状态
		DcDataProcessDesign updateObj = new DcDataProcessDesign();
		updateObj.setId(transJob.getId());
		updateObj.setStatus(updateObj.JOB_STATUS_JOB);	//更新为调度状态
		updateObj.preUpdate();
		dao.update(updateObj);
