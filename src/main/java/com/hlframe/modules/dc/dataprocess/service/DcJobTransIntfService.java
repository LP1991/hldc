...
public class DcJobTransIntfService extends CrudService<DcJobTransIntfDao, DcJobTransIntf> implements DcTaskService {
     ...
     * @params  [entity]
     * @return  void
     * @create by peijd at 2017/4/17 16:06
     */
    @Transactional(readOnly = false)
    public void save(DcJobTransIntfForm form) {
        Assert.notNull(form);
        try{
            //任务ID
            String jobId = StringUtils.isNotBlank(form.getId())?form.getId(): IdGen.uuid();
            //构建接口配置主表
            DcJobTransIntf intf = new DcJobTransIntf();
            MyBeanUtils.copyBean2Bean(intf, form);
            intf.setStatus(DcJobTransData.TASK_STATUS_EDIT);    //编辑状态
            intf.setId(jobId);
            //构建接口来源配置
            DcJobTransIntfSrc src = new DcJobTransIntfSrc();
            MyBeanUtils.copyBean2Bean(src, form);
            src.setJobId(jobId);
            src.setId(jobId);
            src.setParams(StringEscapeUtils.unescapeHtml(src.getParams()));
            //构建接口目标配置
            DcJobTransIntfTar tar = new DcJobTransIntfTar();
            MyBeanUtils.copyBean2Bean(tar, form);
            tar.setId(jobId);
            tar.setJobId(jobId);
            tar.setCreateFlag("N");
     ...
     * @params  [entity]
     * @return  void
     * @create by peijd at 2017/4/18 9:16
     */
    @Override
    @Transactional(readOnly = false)
    public void delete(DcJobTransIntf entity) {
        Assert.hasText(entity.getId());
        entity = dao.get(entity.getId());
        Assert.notNull(entity);

        // 判断任务状态 如果是调度任务  则不可删除
        if(DcJobTransIntf.TASK_STATUS_TASK.equals(entity.getStatus())){
            throw new ServiceException("该任务已添加调度任务, 不可删除!");
        }
        //逻辑删除
        dao.deleteByLogic(entity);
        //TODO: 删除元数据,ES对象及链路关系
    }
     ...
     * @create by peijd at 2017/4/18 10:21
     */
    @Transactional(readOnly = false)
    public String add2Schedule(String jobId) {
        ...
        DcJobTransIntfForm transJob = buildFormData(jobId);
        String taskName = "接口数据采集_"+transJob.getJobName();
        //调度任务
        ...
        //任务来源
        taskMain.setTaskPath(DcTaskMain.TASK_FROMTYPE_EXTRACT_INTF);
        //优先级
        ...
        //更新记录状态
        DcJobTransIntf updateObj = new DcJobTransIntf();
     ...
     */
    @Transactional(readOnly = false)
    public void getAu(DcJobTransIntf obj) {
    ...
    public void saveTarTable(DcJobTransIntfForm form) throws Exception {
        ...
        //目标表 元数据
        DcObjectMain tarMain = new DcObjectMain();					//元数据 主体对象
        tarMain.setId(form.getId());
        tarMain.setJobId(form.getId());							     //任务Id
        ...
        tarMain.setObjDesc(form.getTarName());					//备注信息
