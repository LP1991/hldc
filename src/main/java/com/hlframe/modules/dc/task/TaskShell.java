			...
			Thread syncThread = new Thread(new Runnable() {
				...
				private String callBat(File file) throws Exception {
					...
					try {
						...
						System.out.println("____"+pcs.getErrorStream());
				...
				private String callShell(File file) throws Exception {
					...
					try {
						...
						Process pcs = rt.exec(file.getPath());
						...
						outWriter.flush();
						outWriter.close();
						System.out.println("____"+pcs.getErrorStream().toString());
						result = pcs.exitValue();
					} catch (Exception e) {
						throw new Exception("执行sh文件错误");
					}
					return str;
				}
