...
public class DcJobExportDataService extends CrudService<DcJobExportDataDao, DcJobExportData> implements DcTaskService {
	...
	public DcDataResult runTask(String jobId) throws Exception {
		...
		try {
			...
			result.append("<br>-->调用结果: ").append("0".equals(res.rc)?"成功":"失败");
	 ...
	 * @param jobId
	 * @return
	 * @create by peijd at 2017年2月27日 上午10:54:00
	 */
	@Transactional(readOnly = false)
	public String add2Schedule(String jobId) {
		...
		DcJobExportData exportJob = dao.get(jobId);
		String taskName = "数据导出_"+exportJob.getJobName();
		//调度任务
		...
		//任务描述
		taskMain.setTaskDesc(exportJob.getJobDesc());
		//任务来源
		taskMain.setTaskPath(DcTaskMain.TASK_FROMTYPE_EXPORT_RMDB);
		//优先级
		...
		//更新记录状态
		DcJobExportData updateObj = new DcJobExportData();
